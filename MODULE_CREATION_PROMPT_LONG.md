...
1. **🚨 Mermaidフローチャート構文の厳格遵守（最重要）**
   - **必須開始構文**: テンプレートリテラル \`flowchart TD\` または \`graph TD\` または \`graph LR\`
   - **改行必須**: 開始行の後は必ず改行してからノード定義を開始
   - **絶対禁止**: 括弧 `()` を含むテキストをエッジラベルに使用
   - **絶対禁止**: 【】記号をノードラベルまたはエッジラベルに使用
   - **絶対禁止**: `\n` を使用した改行指定
   - **改行表現**: ノード内改行は `<br/>` ではなく物理的改行のみ使用
   - **判断ノード**: `{}` を使用し、ラベルに「？」を含める
   - **エッジラベル**: シンプルな日本語のみ、記号や括弧は使用禁止
2. **⭐ カラフルで視覚的に魅力的なチャート作成**
   - **色とスタイル**: 各ノードに適切な色とスタイルを設定
   - **区別の明確化**: 処理ノード、判断ノード、結果ノードを色で区別
3. **🚨 キャラクター関係・場所設定の厳格遵守（超重要）**
   - **登場場所の絶対ルール**: キャラクターごとに決められた登場場所を厳守
   - **関係性の正確性**: 片思い関係、友人関係、家族関係を正確に描写
   - **呼び方の統一**: 各キャラクターが他キャラを呼ぶ名前を正確に使用
   - **年齢・学校の一致**: 同じ学校・同じ学年のキャラクターのみ同じ場面に登場させる
4. **文字列内での改行**: Mermaidの場合はテンプレートリテラル内で物理的改行、その他は`\n`使用
5. **テンプレートリテラル使用規則**: Mermaidのみバッククォート（\`）使用、その他はシングルクォート（'）のみ
6. **未定義変数の参照禁止**
7. **無効なJSONオブジェクト構造禁止**
8. **存在しないQ&A（questionsAndAnswers）IDを参照しないこと**
9. **女子中高生らしくない話し方（例:「～わね」「～なのよ」など）を避け、自然な現代日本語で会話文を作成すること**
10. **ストーリー内でキャラクターの専門分野を明示しない**
11. **🚫 参照番号・注釈の絶対禁止**: [3][4][5][6]等の学術論文風注釈を一切使用しないこと
...
- ❌ 【】記号の使用禁止
- ❌ `\n` 改行文字禁止
- ✅ シンプルなエッジラベル: `A -->|契約締結| B`
- ✅ 物理的改行使用: テンプレートリテラル内で実際に改行
- ✅ ノード内改行: `A[契約締結<br/>完了]`
...
**J. 完全な成功例テンプレート（コピー推奨）:**
```javascript
{
  type: 'embed',
  format: 'mermaid',
  title: '法的判断プロセス',
  description: '契約の有効性判断フロー',
  content: `flowchart TD
    A[契約締結] --> B{有効要件<br/>充足？}
    B -->|YES| C[有効な契約]
    B -->|NO| D[無効な契約]
...
```javascript
{
  ...
  format: 'html',
  title: '条文比較表',
  description: '民法と商法の比較',
  content: '<table>...</table>' // 詳細はAIが適切に作成
}
```
...
**◆ みかんの高校の友人・関係者**
- **しみちゃん**: みかんの大親友、17歳。本名は清水。刑事訴訟法のスペシャリスト。常に冷静沈着で、どこか達観した大人びた雰囲気を持つ。**非常にサバサバした性格**で、感情的になることは稀。みかんの最大の理解者であり、彼女の恋や悩みの相談に乗ることが多い。しばしば核心を突く鋭い一言を放つ。みかんを「みかん」と呼ぶ。主にみかんの高校に登場。**ベア研との関係**: ベア研メンバーとのかかわりはほとんどない。
...
- **ミエ**: みかんの高校のクラスメート、17歳。特許法に詳しい。発明や技術に関する話題になると目を輝かせる。普段はクールで落ち着いているが、知的財産の話になると熱くなる。将来は弁理士を目指している。みかんを「みかん」と呼ぶ。主にみかんの高校に登場。
...
- **ナスオ**: ユズヒコの友人、14歳。本名は那須野。物権法に詳しい。ナスのような顔の形が特徴。常にテンションが高く、他人の話を自己流に解釈して噂を広めるトラブルメーカーな一面も。標準語で話し、絶対に敬語は使わない。ユズヒコを「ユズピ」と呼ぶ。主にユズヒコの中学校に登場。
...
- **原先生**: ユズヒコの中学の担任教師。通称「ハラセン」。５０代のオバサンで一人称は「私」。民事の授業を担当。元裁判官で民事実務に精通している。ギラギラした目つきとダミ声が特徴で、生徒から恐れられている。給食の食べ残しを許さないなど非常に厳しいが、事なかれ主義な一面も。ユズヒコの中学校に登場。【セリフ例】給食、残さず食べる。これ常識。　君たち何をしている。
...
**D. ユズヒコの中学校での登場制限**
- **✅ 登場可能**: ユズヒコ、藤野、ナスオ、石田、山下、川島、須藤、原先生
...
- **片思い相手には特別な呼び方**（例：みかんは「岩城くん」、川島は「タチバナくん」）
...
- **条文記載**: `{{【民法177条】}}`の形式（【】の外に{{}}）
- **判例年月日**: 暗記不要のため空欄化禁止
- **重要概念**: 必ず{{}}で空欄化（例：`{{時効取得}}`、`{{原始取得}}`）
- **空欄の量**: 1つのanswerになるべくたくさんの単語単位の空欄を作成
- **空欄対象**: 法的概念、結論、理由、要件、効果など重要部分すべて
...
questionsAndAnswers: [
  { 
    ...
    rank: 'A', 
    question: '○○について説明しなさい。', 
    answer: '○○とは、{{重要概念}}による{{法的効果}}である。要件は{{要件1}}と{{要件2}}であり、{{【民法○条】}}に規定される。結果として{{結論}}となる。' 
  }
],
```
...
これらは絶対に言葉遣い等を変えずに、そのまま反映させなさい！また、空欄（{{}}による）は司法試験対策上重要な点にたくさん設けなさい！


では参りましょう。まずは、最初から　   storyまでをかけ。
 story: [
    { type: ???, text: ???
    ・・・・ //最低50回のやり取り、それ以上あればなおとても良い
    ????。",
    },
また、ミニ論文は、絶対にお前が考えてはならず、できれば判例の事例、なければ教科書事例をそのまま用いて、変えるのは登場人物をあたしンちキャラに変えるだけにしろ。
そして、私が「続き」と言ったらexplanation:以降の続きを書きなさい。
ぜったいにこの後示すquestionsAndAnswersを絶対に省略したり、意味を変えたりしてはならない。これは絶対的だ。
また、これに加えてリサーチを重ねて、最新の情報を増強しろ