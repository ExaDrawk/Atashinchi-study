...
// ★★★ サポート対象法律リスト ★★★
const SUPPORTED_LAWS = ['民法', '会社法', '刑法', '商法', '日本国憲法', '行政手続法', '刑事訴訟法', '刑事訴訟規則', '民事訴訟規則'];
...
function kanjiToArabic(kanji) {
    if (!kanji || typeof kanji !== 'string') return NaN;
    
    // 数字のみの場合はそのまま返す
    if (/^\d+$/.test(kanji)) return parseInt(kanji);
    
    // 「の」を含む場合（例: 「413の2」）
    if (kanji.includes('の')) {
        const parts = kanji.split('の');
        if (parts.length === 2) {
            const main = kanjiToArabic(parts[0]);
            const sub = kanjiToArabic(parts[1]);
            return main + '_' + sub;
        }
    }
    ...
    for (let i = 0; i < kanji.length; i++) {
        ...
        if (value !== undefined) {
            if (value >= 10) {
                // 単位（十、百、千、万）の場合
                if (temp === 0) temp = 1; // 「十」のように数字が省略されている場合
                result += temp * value;
                temp = 0;
            } else {
                // 数字（一～九）の場合
...
export function getTextContent(node) {
    ...
    // ★★★ Rubyタグ対応を最優先: <Ruby>瑕疵<Rt>かし</Rt></Ruby> → 瑕疵（かし）
    if (node.Ruby) {
        console.log(`🔍 Ruby要素発見:`, JSON.stringify(node, null, 2));
        
        let result = '';
        
        // _プロパティがあればそれを最初に追加（括弧を調整）
        if (node._ && typeof node._ === 'string') {
            let baseText = node._;
            // 「（代理行為の）」→「（代理行為の」に変換（閉じ括弧を削除）
        ...
        // Ruby要素を処理
        const rubyArray = Array.isArray(node.Ruby) ? node.Ruby : [node.Ruby];
        rubyArray.forEach(rubyItem => {
            let base = '';
            let rt = '';
            
            // ベーステキストを取得
            if (typeof rubyItem === 'string') {
                base = rubyItem;
            } else if (rubyItem._) {
                base = rubyItem._;
            }
            ...
            console.log(`🔍 Ruby処理結果: base="${base}", rt="${rt}"`);
            
            // ベーステキストとルビテキストを組み合わせ
            if (base && rt) {
                result += `${base}（${rt}）`;
            } else if (base) {
                result += base;
        ...
        // 最後に閉じ括弧を追加
        if (node._ && typeof node._ === 'string' && node._.includes('（')) {
            result += '）';
        }
...
function parseInputText(inputText, supportedLaws) {
    ...
    console.log(`🔍 条文解析開始: "${normalizedInput}" (元の入力: "${inputText}")`);
    console.log(`🔍 サポート対象法令: ${supportedLaws.join(', ')}`);
    ...
    // ★★★ パターン2-ただし書: 「民法295条1項ただし書」形式 ★★★
    const patternTadashi = new RegExp(`^(${lawPattern})(\\d+)条(\\d+)項ただし書$`);
    const matchTadashi = patternTadashi.exec(normalizedInput);
    if (matchTadashi) {
        return {
            success: true,
            lawName: matchTadashi[1],
            articleNumber: matchTadashi[2],
            paragraphNumber: matchTadashi[3],
            segment: 'ただし書'
        };
    }

    // ★★★ パターン2-ただし書（条のみ）: 「民法295条ただし書」形式 ★★★
    const patternTadashiOnly = new RegExp(`^(${lawPattern})(\\d+)条ただし書$`);
    const matchTadashiOnly = patternTadashiOnly.exec(normalizedInput);
    if (matchTadashiOnly) {
        return {
            success: true,
            lawName: matchTadashiOnly[1],
            articleNumber: matchTadashiOnly[2],
            segment: 'ただし書'
        };
    }

    // ★★★ パターン2-ただし書（第項）: 「民法295条第1項ただし書」形式 ★★★
    const patternTadashiDai = new RegExp(`^(${lawPattern})(\\d+)条第(\\d+)項ただし書$`);
    const matchTadashiDai = patternTadashiDai.exec(normalizedInput);
    if (matchTadashiDai) {
        return {
            success: true,
            lawName: matchTadashiDai[1],
            articleNumber: matchTadashiDai[2],
            paragraphNumber: matchTadashiDai[3],
            segment: 'ただし書'
        };
    }

    // ★★★ パターン2-ただし書（項号）: 「民法295条1項第2号ただし書」形式 ★★★
    const patternTadashiItem = new RegExp(`^(${lawPattern})(\\d+)条(\\d+)項第?(\\d+)号ただし書$`);
    const matchTadashiItem = patternTadashiItem.exec(normalizedInput);
    if (matchTadashiItem) {
        return {
            success: true,
            lawName: matchTadashiItem[1],
            articleNumber: matchTadashiItem[2],
            paragraphNumber: matchTadashiItem[3],
            itemNumber: matchTadashiItem[4],
            segment: 'ただし書'
        };
    }

    // ★★★ パターン2-イロハ: 「行政手続法13条1項1号イ」形式 ★★★
    const patternIroha = new RegExp(`^(${lawPattern})(\\d+)条(\\d+)項(\\d+)号([イロハニホヘトチリヌルヲワカヨタレソツネナラムウヰノオクヤマケフコエテアサキユメミシヱヒモセス])$`);
    const matchIroha = patternIroha.exec(normalizedInput);
    if (matchIroha) {
        return {
            success: true,
            lawName: matchIroha[1],
            articleNumber: matchIroha[2],
            paragraphNumber: matchIroha[3],
            itemNumber: matchIroha[4],
            subItem: matchIroha[5]
        };
    }

    // ★★★ パターン2-本文: 「刑事訴訟法197条1項本文」形式 ★★★
    const patternHonbun = new RegExp(`^(${lawPattern})(\\d+)条(\\d+)項本文$`);
    const matchHonbun = patternHonbun.exec(normalizedInput);
    if (matchHonbun) {
        return {
            success: true,
            lawName: matchHonbun[1],
            articleNumber: matchHonbun[2],
            paragraphNumber: matchHonbun[3],
            segment: '本文'
        };
    }

    // ★★★ パターン2-本文（条のみ）: 「刑事訴訟法197条本文」形式 ★★★
    const patternHonbunOnly = new RegExp(`^(${lawPattern})(\\d+)条本文$`);
    const matchHonbunOnly = patternHonbunOnly.exec(normalizedInput);
    if (matchHonbunOnly) {
        return {
            success: true,
            lawName: matchHonbunOnly[1],
            articleNumber: matchHonbunOnly[2],
            segment: '本文'
        };
    }
    ...
    return { 
        success: false, 
        error: `対応していない形式: "${normalizedInput}" (元の入力: "${inputText}")`,
        supportedFormats: [
            "民法1条",
            "民法2条",
            "民法548条の2",
...
export async function saveLawXMLWithFileName(lawName, xmlText, lawId) {
    try {
        ...
        if (DEBUG_LOGS) console.log(`💾 XMLファイル保存: ${fileName} (${Math.round(xmlText.length / 1024)}KB)`);
        return fileName;
    } catch (error) {
...
export async function getLawIdFromAPI(lawName, retryCount = 0) {
    ...
    try {
        if (DEBUG_LOGS) console.log(`🔍 法令ID検索: ${lawName} (試行 ${retryCount + 1}/${maxRetries + 1})`);
        
        const mappedLawName = LAW_NAME_MAPPING[lawName] || lawName;
        if (DEBUG_LOGS) console.log(`🔄 法令名マッピング: ${lawName} → ${mappedLawName}`);
        ...
        for (const pattern of searchPatterns) {
            if (DEBUG_LOGS) console.log(`🔍 検索パターン: "${pattern}"`);
            ...
            try {
                ...
                if (DEBUG_LOGS) console.log(`📄 XMLレスポンス長: ${xmlText.length}文字`);
                ...
                if (DEBUG_LOGS) console.log(`📋 発見された法令数: ${laws.length}`);
                ...
                for (const law of laws) {
                    ...
                    if (foundLawName && lawId) {
                        ...
                        if (isStrictExactMatch(lawName, foundLawName) || isStrictExactMatch(mappedLawName, foundLawName)) {
                            ...
                            if (DEBUG_LOGS) console.log(`🎯 厳密完全一致発見: ${lawName} → ${foundLawName} (ID: ${lawId})`);
                        }
                        // 従来の完全一致をチェック
                        ...
                        // 部分一致をチェック
                            ...
                            if (DEBUG_LOGS) console.log(`📝 部分一致発見: ${lawName} → ${foundLawName} (ID: ${lawId})`);
                        }
                    }
                }
                ...
                if (exactMatches.length > 0) {
                    ...
                    if (DEBUG_LOGS) console.log(`✅ 完全一致を採用: ${lawName} → ${match.foundLawName} (ID: ${match.lawId})`);
                    return match.lawId;
                }
...
export async function ensureLawXMLByFileName(lawName, existingFiles) {
    ...
    if (cachedFileName && existingFiles.has(cachedFileName)) {
        if (DEBUG_LOGS) console.log(`📋 キャッシュヒット: ${lawName} → ${cachedFileName}`);
    ...
    for (const [fileName, xmlText] of existingFiles) {
        ...
        if (extracted && extracted.lawName === lawName) {
            lawFileNameCache.set(lawName, fileName);
            if (DEBUG_LOGS) console.log(`📋 ファイル名マッチ: ${lawName} → ${fileName}`);
            return xmlText;
        }
    }
    
    try {
        if (DEBUG_LOGS) console.log(`🌐 API取得開始: ${lawName}`);
        ...
        const xmlText = await response.text();
        if (DEBUG_LOGS) console.log(`📄 法令データ取得完了: ${lawName} (${Math.round(xmlText.length / 1024)}KB)`);
        
        const fileName = await saveLawXMLWithFileName(lawName, xmlText, lawId);
        existingFiles.set(fileName, xmlText);
        
        if (DEBUG_LOGS) console.log(`✅ API取得完了: ${lawName} → ${fileName}`);
        return xmlText;
...
export async function updateAllSupportedLaws(supportedLaws, existingFiles) {
    ...
    for (let i = 0; i < supportedLaws.length; i++) {
        ...
        try {
            if (DEBUG_LOGS) console.log(`${progress} 処理中: ${lawName}`);
            await ensureLawXMLByFileName(lawName, existingFiles);
            
            results.push({ lawName, status: 'success' });
            successCount++;
            if (DEBUG_LOGS) console.log(`${progress} ✅ 完了: ${lawName}`);
...
export function formatArticleNode(articleNode, paragraphNumber = null, itemNumber = null, segment = null, subItem = null) {
    let content = '';
    ...
    try {
        ...
        if (articleNode.Paragraph) {
            ...
            content += formatParagraphs(articleNode.Paragraph, paragraphNumber, itemNumber, segment, subItem);
        } else {
...
// ★★★ 前段・後段・ただし書のハイライト処理関数 ★★★
function processSegmentHighlight(text, segment) {
    console.log(`🎯 セグメント処理開始: segment="${segment}", text="${text}"`);
    
    // 「ただし書」の処理
    if (segment === 'ただし書') {
        if (DEBUG_LOGS) console.log(`📝 ただし書処理開始`);
        
        // 「ただし、」または「但し、」で始まる部分を検索（ひらがな・漢字両対応）
        const tadashiPattern = /(ただし、|但し、)(.+)/;
        const match = text.match(tadashiPattern);
        
        if (match) {
            const tadashiKeyword = match[1]; // 「ただし、」または「但し、」
            const beforeTadashi = text.substring(0, text.indexOf(tadashiKeyword));
            const tadashiPart = tadashiKeyword + match[2];
            console.log(`🎯 ただし書発見: "${tadashiPart}"`);
            
            const highlightedTadashi = `<span class="font-bold text-blue-700 bg-yellow-300 px-1 rounded">${tadashiPart}</span>`;
            return beforeTadashi + highlightedTadashi;
        } else {
            console.log(`⚠️ ただし書が見つかりません: "${text}"`);
            return text;
        }
    }

    // 「本文」の処理（ただし書以外の部分をハイライト）
    if (segment === '本文') {
        if (DEBUG_LOGS) console.log(`📝 本文処理開始`);
        
        // 「ただし、」または「但し、」で始まる部分を検索
        const tadashiPattern = /(ただし、|但し、)(.+)/;
        const match = text.match(tadashiPattern);
        ...
        if (match) {
            ...
            const beforeTadashi = text.substring(0, text.indexOf(tadashiKeyword));
            const tadashiPart = tadashiKeyword + match[2];
            console.log(`🎯 本文処理: ただし書きを除外して本文をハイライト`);
            
            // 本文部分（ただし書き以外）をハイライト
            const highlightedHonbun = `<span class="font-bold text-blue-700 bg-yellow-300 px-1 rounded">${beforeTadashi}</span>`;
            return highlightedHonbun + tadashiPart;
        } else {
            // ただし書きがない場合は全体を本文としてハイライト
            console.log(`🎯 本文処理: ただし書きなし、全体をハイライト`);
            return `<span class="font-bold text-blue-700 bg-yellow-300 px-1 rounded">${text}</span>`;
        }
    }
    
    // 句点（。）で文を分割
    const sentences = text.split('。').filter(sentence => sentence.trim().length > 0);
    if (DEBUG_LOGS) console.log(`📝 分割された文: ${sentences.length}個`, sentences);
    ...
    console.log(`✅ セグメント処理完了: "${result}"`);
    return result;
}

// ★★★ 段落を整形（完全修正版・explicitArray: true・前段後段対応） ★★★
function formatParagraphs(paragraphs, targetParagraph, targetItem, segment, subItem) {
    let content = '';
                ...
                paragraphSentences.forEach((paragraphSentence, index) => {
                    ...
                    if (paragraphSentence.Sentence) {
                        ...
                        sentenceArray.forEach((sent, sentIndex) => {
                            ...
                            if (DEBUG_LOGS) console.log(`📝 Sentence ${sentIndex} 抽出: "${sentenceText}"`);
                            paragraphText += sentenceText;
                        });
                    } else {
                        // ParagraphSentence自体にテキストが含まれる場合
                        const sentenceText = getTextContent(paragraphSentence);
                        if (DEBUG_LOGS) console.log(`📝 ParagraphSentence直接テキスト: "${sentenceText}"`);
                        paragraphText += sentenceText;
                    }
                });
                
                // ★★★ 前段・後段・ただし書・本文の処理 ★★★
                if (segment && (segment === '前段' || segment === '後段' || segment === 'ただし書' || segment === '本文')) {
                ...
                items.forEach((item, itemIndex) => {
                    ...
                    if (item.Subitem1) {
                        ...
                        subitems.forEach(subitem => {
                            ...
                            // ★★★ イロハニ文字の指定があるかチェック ★★★
                            const isTargetSubItem = isTargetItem && subItem && subitemNum === subItem;
                            console.log(`🎯 イロハニマーキング判定: subItem="${subItem}", subitemNum="${subitemNum}", isTargetSubItem=${isTargetSubItem}`);
                            
                            if (isTargetSubItem) {
                                console.log(`✅ イロハニマーキング開始: SubItem ${subitemNum}`);
                                content += `\n　　<span style="background-color: #fde047; padding: 2px 4px; border-radius: 3px; font-weight: bold;">`;
                            } else {
                                content += '\n　　';
                            }
                            ...
                            // ★★★ イロハニ文字指定時の強調表示終了タグ ★★★
                            if (isTargetSubItem) {
                                console.log(`✅ イロハニマーキング終了: SubItem ${subitemNum}`);
                                content += '</span>';
                            }
                        });
                    }
...
export async function getFormattedArticle(lawName, articleNumber, paragraphNumber = null, itemNumber = null, segment = null, subItem = null, existingFiles = new Map()) {
    try {
        console.log(`📖 条文取得開始: ${lawName} 第${articleNumber}条${paragraphNumber ? ` 第${paragraphNumber}項` : ''}${itemNumber ? ` ${itemNumber}号` : ''}${segment ? ` ${segment}` : ''}`);
        
        // ★★★ 特別な法令（テキスト形式）の処理 ★★★
        if (lawName === '刑事訴訟規則' || lawName === '民事訴訟規則') {
            return await getFormattedArticleFromTextFile(lawName, articleNumber, existingFiles);
        }
        ...
        const formattedText = formatArticleNode(articleNode, paragraphNumber, itemNumber, segment, subItem);
...
export async function parseAndGetArticle(inputText, supportedLaws = null, existingFiles = new Map()) {
    const lawsList = supportedLaws || SUPPORTED_LAWS;
    ...
    const { lawName, articleNumber, paragraphNumber, itemNumber, segment, subItem } = parseResult;
    
    try {
        const articleText = await getFormattedArticle(lawName, articleNumber, paragraphNumber, itemNumber, segment, subItem, existingFiles);
        return articleText;
    } catch (error) {
        return `❌ 条文の取得中にエラーが発生しました: ${error.message}`;
    }
...
// ★★★ XMLをテキストに変換（タグ除去） ★★★
function convertXmlToText(xmlContent) {
    try {
        // XML宣言とDOCTYPEを除去
        let text = xmlContent.replace(/<\?xml.*?\?>|<!DOCTYPE.*?>|<!--.*?-->|<xml.*?>|<\/xml>/gi, '');
        ...
        console.log(`🔄 XML変換結果: ${text.length}文字のテキストに変換`);
        return text;
    } catch (error) {
        console.error('❌ XML→テキスト変換エラー:', error);
        return xmlContent; // エラー時は元のコンテンツを返す
    }
}

// ★★★ テキスト形式の規則ファイルから条文を取得 ★★★
async function getFormattedArticleFromTextFile(lawName, articleNumber, existingFiles = new Map()) {
    try {
        console.log(`📖 テキスト形式条文取得開始: ${lawName} 第${articleNumber}条`);
        
        // ファイル名を決定
        const fileName = `${lawName}.xml`;
        const filePath = path.join(__dirname, 'laws', fileName);
        ...
        try {
            ...
            return `❌ 法令ファイルが見つかりません: ${fileName}`;
        }
        
        // ファイルを読み込み
        const fileContent = await fs.readFile(filePath, 'utf8');
        console.log(`📄 テキストファイル読み込み完了: ${fileContent.length}文字`);
        
        // XMLファイルをテキストに変換（タグ除去）
        const textContent = convertXmlToText(fileContent);
        console.log(`📝 XML→テキスト変換完了: ${textContent.length}文字`);
        
        // 条文をパース
        const articleData = parseTextLawFile(textContent, articleNumber);
        ...
        // 指定された形式でフォーマット
        const formattedText = `（（${articleData.title}））\n${articleData.articleNumber} ${articleData.content}`;
        
        console.log(`✅ テキスト形式条文取得成功: ${lawName} 第${articleNumber}条`);
        return formattedText;
        
    } catch (error) {
        console.error(`❌ テキスト形式条文取得エラー (${lawName} 第${articleNumber}条):`, error.message);
        return `❌ 条文の取得中にエラーが発生しました: ${error.message}`;
    }
}

// ★★★ テキスト形式の法令ファイルをパース ★★★
function parseTextLawFile(fileContent, targetArticleNumber) {
    const lines = fileContent.split('\n');
    let currentTitle = '';
    let currentArticleNumber = '';
    let currentContent = '';
    let isCollectingContent = false;
    let pendingTitle = ''; // 保留中のタイトル（次の条文で使用）
    
    console.log(`🔍 parseTextLawFile開始: 対象条文=${targetArticleNumber}, 総行数=${lines.length}`);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        console.log(`🔍 処理中行 ${i}: "${line.substring(0, 50)}..."`);
        
        // タイトル行の検出（（...法第[数字]条）パターン）
        const titleMatch = line.match(/（(.+?)・法第([一二三四五六七八九十百千万\d]+)条）/);
        if (titleMatch) {
            const titleText = titleMatch[1];
            const lawArticleNumber = titleMatch[2];
            const arabicLawNumber = kanjiToArabic(lawArticleNumber);
            
            console.log(`🏷️ タイトル検出: "${titleText}", 法条文=${lawArticleNumber}(${arabicLawNumber})`);
            
            if (!isNaN(arabicLawNumber)) {
                pendingTitle = titleText;
                // このタイトルが対象条文のものかチェック
                if (arabicLawNumber.toString() === targetArticleNumber.toString()) {
                    console.log(`✅ 対象条文のタイトル発見: 第${targetArticleNumber}条`);
        ...
        // 条文番号の検出（第一条、第二条、第三百五条など）
        const articleMatch = line.match(/^第([一二三四五六七八九十百千万\d]+)条/);
        if (articleMatch) {
            console.log(`📝 条文番号検出: "${articleMatch[0]}"`);
            
            // 前の条文の内容を保存
            if (currentArticleNumber && currentContent) {
                console.log(`📝 条文完了: 第${currentArticleNumber}条, 内容長=${currentContent.length}`);
                
                if (currentArticleNumber === targetArticleNumber.toString()) {
                    console.log(`✅ 対象条文発見: 第${currentArticleNumber}条`);
                    return {
                        title: currentTitle,
                        articleNumber: `第${currentArticleNumber}条`,
            ...
            // 新しい条文を開始
            const kanjiNumber = articleMatch[1];
            const arabicNumber = kanjiToArabic(kanjiNumber);
            console.log(`🔢 漢数字変換: "${kanjiNumber}" → ${arabicNumber}`);
            
            if (isNaN(arabicNumber)) {
                console.log(`❌ 変換失敗: "${kanjiNumber}"`);
                continue;
            }
            
            currentArticleNumber = arabicNumber.toString();
            currentContent = line.substring(articleMatch[0].length).trim();
            
            // 保留中のタイトルがある場合は使用
            if (pendingTitle) {
                currentTitle = pendingTitle;
                pendingTitle = '';
                console.log(`📋 保留タイトル適用: ${currentTitle}`);
            }
            
            isCollectingContent = true;
            console.log(`📖 新規条文開始: 第${currentArticleNumber}条`);
            continue;
        }
        ...
        if (isCollectingContent) {
            // 次の条文の開始をチェック
            const nextArticleMatch = line.match(/^第[一二三四五六七八九十百千万\d]+条/);
            if (nextArticleMatch) {
                // 次の条文が始まったら、現在の内容をチェック
                console.log(`🔄 次条文検出: 現在の条文=${currentArticleNumber}, 対象=${targetArticleNumber}`);
                
                if (currentArticleNumber === targetArticleNumber.toString()) {
                    console.log(`✅ 対象条文完了: 第${currentArticleNumber}条`);
                    return {
                        title: currentTitle,
                        articleNumber: `第${currentArticleNumber}条`,
                        content: currentContent.trim()
                ...
                // 新しい条文を開始
                const newKanjiNumber = nextArticleMatch[1];
                const newArabicNumber = kanjiToArabic(newKanjiNumber);
                console.log(`🔢 次条文変換: "${newKanjiNumber}" → ${newArabicNumber}`);
                
                if (!isNaN(newArabicNumber)) {
                    currentArticleNumber = newArabicNumber.toString();
                    currentContent = line.substring(nextArticleMatch[0].length).trim();
                    
                    // 保留中のタイトルがある場合は使用
                    if (pendingTitle) {
                        currentTitle = pendingTitle;
                        pendingTitle = '';
                        console.log(`📋 次条文に保留タイトル適用: ${currentTitle}`);
                    }
                    
                    console.log(`📖 次条文開始: 第${currentArticleNumber}条`);
                }
                continue;
            }
            
            // タイトル行の検出（括弧で囲まれた行）
            if (line.startsWith('（') && line.endsWith('）') && !line.includes('条')) {
                // 括弧の数をカウントして適切に除去
                let titleText = line;
                while (titleText.startsWith('（') && titleText.endsWith('）')) {
                    titleText = titleText.slice(1, -1);
                }
                currentTitle = titleText;
                console.log(`🏷️ タイトル検出: ${currentTitle}`);
                continue;
            }
    ...
    // 最後の条文をチェック
    console.log(`🏁 最終チェック: 現在の条文=${currentArticleNumber}, 対象=${targetArticleNumber}`);
    
    if (currentArticleNumber === targetArticleNumber.toString()) {
        console.log(`✅ 最終条文発見: 第${currentArticleNumber}条`);
        return {
            title: currentTitle,
            articleNumber: `第${currentArticleNumber}条`,
            content: currentContent.trim()
    ...
    console.log(`❌ 条文が見つかりません: 第${targetArticleNumber}条`);
    return null;
}

export { parseTextLawFile };
