...
export function getSpeedFilterPanelHTML() {
    ...
    return `
        ...
        <!-- 該当条文プレビューエリア -->
        <div id="speed-article-preview" class="mt-4">
            <p class="text-sm font-semibold text-gray-700 mb-2">該当条文 <span id="speed-article-count" class="text-purple-600">(読込中...)</span></p>
            <div id="speed-article-buttons" class="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto p-2 bg-gray-50 rounded-lg border border-gray-200" style="scrollbar-width:thin;">
...
function updateSpeedFilterSummary() {
    ...
    // 条文プレビューも更新
    updateSpeedArticlePreview();
}

// 該当条文をプレビュー表示する関数
let articlePreviewCache = null;
async function updateSpeedArticlePreview() {
    const container = document.getElementById('speed-article-buttons');
    ...
    try {
        ...
        if (articles.length === 0) {
            container.innerHTML = '<span class="text-gray-400 text-sm">該当する条文がありません</span>';
            if (countSpan) countSpan.textContent = '(0件)';
            return;
        }
        ...
        // ソート（法令名→条番号順）
        articles.sort((a, b) => {
            if (a.lawName !== b.lawName) return a.lawName.localeCompare(b.lawName);
            return parseInt(a.articleNumber) - parseInt(b.articleNumber);
        });

        const buttons = articles.map(a => {
            const lawColor = getCategoryColor(a.lawName);
            const shortLawName = a.lawName.length > 4 ? a.lawName.substring(0, 3) + '…' : a.lawName;
            return `<button class="speed-preview-btn" style="background:${lawColor.bgColor};color:${lawColor.color};border:none;padding:0.25rem 0.5rem;border-radius:0.375rem;font-size:0.75rem;font-weight:bold;cursor:pointer;white-space:nowrap;transition:transform 0.1s,box-shadow 0.1s;" title="${a.lawName}${a.articleNumber}条" data-law="${a.lawName}" data-article="${a.articleNumber}">${shortLawName}${a.articleNumber}条</button>`;
        }).join('');

        container.innerHTML = buttons;
        if (countSpan) countSpan.textContent = `(${articles.length}件)`;

        // ボタンクリック時の条文表示とホバー効果
        container.querySelectorAll('.speed-preview-btn').forEach(btn => {
            ...
            btn.addEventListener('click', () => {
                ...
                const articleNumber = btn.dataset.article;
                showArticleModalFromHome({ lawName, articleNumber });
            });
        });

    } catch (error) {
        console.error('条文プレビュー更新エラー:', error);
        container.innerHTML = '<span class="text-red-400 text-sm">読み込みエラー</span>';
        if (countSpan) countSpan.textContent = '(エラー)';
    }
}

// 条文表示モーダル（ホームページ用）
async function showArticleModalFromHome(article) {
    // 既存のモーダルがあれば削除
    const existing = document.getElementById('home-article-modal');
    if (existing) existing.remove();

    // モーダル作成
    const modal = document.createElement('div');
    modal.id = 'home-article-modal';
    ...
    modal.innerHTML = `
        <div style="background:#1e293b;border-radius:1rem;max-width:800px;width:100%;max-height:80vh;overflow:auto;padding:1.5rem;position:relative;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);">
            <button id="home-modal-close" style="position:absolute;top:1rem;right:1rem;background:transparent;border:none;color:#9ca3af;font-size:1.5rem;cursor:pointer;padding:0.5rem;line-height:1;">&times;</button>
            ...
            <div id="home-modal-content" style="color:#e5e7eb;font-size:1rem;line-height:1.8;">
    ...
    // 閉じるボタン
    document.getElementById('home-modal-close').addEventListener('click', () => modal.remove());
    ...
    // 条文取得
    try {
        const txt = `${article.lawName}${article.articleNumber}条`;
        ...
        const contentDiv = document.getElementById('home-modal-content');
        if (contentDiv) {
            // （（見出し））を強調表示に変換
            let formatted = content
                .replace(/（（(.+?)））/g, '<div style="font-weight:bold;font-size:1.15rem;color:#fbbf24;margin:0.5rem 0;">$1</div>')
                .replace(/\n/g, '<br>');
