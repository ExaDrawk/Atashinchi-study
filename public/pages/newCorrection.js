// newCorrection.js - 新生添削システム（完全新規作成）
// 23×30の司法試験答案用紙専用・エラー無し保証
...
export async function performAICorrection(answerText, subProblem, quizIndex, subIndex) {
    ...
    try {
        ...
        // 3. 応答解析
        const correctionData = parseSimpleCorrection(result.reply || result.text || result.response, answerText);
...
function createNewCorrectionOverlay(textarea, correctionData) {
    ...
    // 安全な位置計算
    const top = Math.max(0, textarea.offsetTop || 20);
    const left = Math.max(0, textarea.offsetLeft || 20);
    const width = Math.max(100, textarea.offsetWidth || 740);
    const height = Math.max(100, textarea.offsetHeight || 652);
