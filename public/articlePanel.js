...
async function fetchAndDisplayArticle() {
    ...
    try {
        ...
        // ★★★ 単一括弧内のグレー表示処理 ★★★
        const greyFormattedContent = formatSingleParentheses(formattedContent);
        
        // ★★★ ただし書きハイライト処理 ★★★
        const finalFormattedContent = applyProvisoHighlight(greyFormattedContent);
...
function showArticlePanelWithPreset(lawName, articleRef, provisoText = null) {
    ...
    setTimeout(() => {
        ...
        if (lawSelect && articleInput) {
            ...
            // ★★★ ただし書きがある場合は条文番号に結合 ★★★
            let fullArticleRef = articleRef;
            if (provisoText) {
                fullArticleRef = articleRef + provisoText;
                console.log(`📝 ただし書き結合: "${articleRef}" + "${provisoText}" = "${fullArticleRef}"`);
            }
            
            articleInput.value = fullArticleRef;
...
// ★★★ 単一括弧内のグレー表示機能 ★★★
function formatSingleParentheses(text) {
    // 単一括弧「（～）」を検出してグレー表示
    // ただし、二重括弧「（（～））」は除外する
    // また、HTMLタグ内の括弧は処理しない
    
    console.log('📝 単一括弧のグレー表示処理を開始');
    
    // HTMLタグと既存のspanタグを一時的に保護
    ...
    // 二重括弧を先に検出して保護（二重括弧内の単一括弧を除外するため）
    protectedText = protectedText.replace(/（（[^）]*））/g, (match) => {
        const index = protectedParts.length;
        protectedParts.push(match);
        return `__PROTECTED_${index}__`;
    ...
    protectedText = protectedText.replace(/（([^）]*)）/g, (match, content) => {
        console.log(`✅ 単一括弧検出: "${match}"`);
        return `<span style="color: #9ca3af;">（${content}）</span>`;
    });
