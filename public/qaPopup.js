...
import { generateUnifiedStatusButtons, toggleHintDisplay, copyQAContent } from './qaRenderer.js';
...
/**
 * ランクバッジのHTMLを生成
 * @param {string} rank - ランク文字列 (S, A, B, C等)
 * @returns {string} ランクバッジHTML
 */
function createRankBadge(rank) {
    if (!rank) return '';
    
    const rankUpper = rank.replace(/ランク$/,'').replace(/\s/g,'').toUpperCase();
    // homePage.js の getRankColor を使って色を一元化
    let rankInfo = null;
    if (typeof window.getRankColor === 'function') {
        rankInfo = window.getRankColor(rankUpper);
    }
    ...
    if (!rankInfo) {
        ...
        rankInfo = fallback[rankUpper] || { color: '#6b7280', bgColor: '#f9fafb', borderColor: '#d1d5db' };
    }

    // 表示はユーザー要望に合わせて "ランク" を付けず、インラインスタイルで完全一致させる
    return `<span class="inline-block px-2 py-0.5 rounded text-xs font-bold border mr-2" style="color: ${rankInfo.color}; background-color: ${rankInfo.bgColor}; border-color: ${rankInfo.borderColor};">${rankUpper}</span>`;
}
 ...
 */
async function recreateQAPopup({ popupId, qaIndex, qNumber, quizIndex, subIndex }) {
    ...
    console.log('🔍 Q&A問題文の条文参照処理開始 (qaPopup.js):', qa.question.substring(0, 100) + '...');
    let qaQuestion = processArticleReferences(qa.question, window.SUPPORTED_LAWS || []);
    console.log('✅ Q&A問題文の条文参照処理完了 (qaPopup.js):', qaQuestion.substring(0, 100) + '...');
    
    // 先にanswerの{{}}の外の【】を条文参照ボタン化してから、空欄化処理を行う
    console.log('🔍 Q&A解答部の条文参照処理開始 (qaPopup.js):', qa.answer.substring(0, 100) + '...');
    let qaAnswerWithArticleRefs = processArticleReferences(qa.answer, window.SUPPORTED_LAWS || []);
    console.log('✅ Q&A解答部の条文参照処理完了 (qaPopup.js):', qaAnswerWithArticleRefs.substring(0, 100) + '...');
    let qaAnswer = processBlankFillText(qaAnswerWithArticleRefs, `qa-recreate-${qaIndex}`, qa.id);
    ...
    // ランクバッジを生成
    const rankBadge = createRankBadge(qa.rank);
...
/**
 * Q&AポップアップのHTMLを生成（統一ステータスボタン対応・ランク表示対応）
 * @param {string} popupId - ポップアップID
 * @param {number} qNumber - Q番号
 * @param {string} qaQuestion - 質問内容
 * @param {string} qaAnswer - 回答内容
 * @param {string} statusButtons - ステータスボタンHTML（事前生成済み）
 * @param {string} rankBadge - ランクバッジHTML（事前生成済み）
            ...
            <div class="mb-2">
                ...
                <div class="qa-answer-content bg-green-50 p-3 rounded-lg border border-green-200">
                    <div class="flex gap-2 mb-2">
                        <button type="button" class="hint-qa-btn bg-blue-100 hover:bg-blue-200 text-blue-800 font-bold py-1 px-2 rounded border border-blue-300 text-xs">💡 ヒント！</button>
...
function setupQAPopupEvents(popupId) {
    ...
    // ★★★ 条文ボタンのイベントリスナーを設定 ★★★
    console.log(`🔧 Q&Aポップアップ内の条文ボタン設定: ${popupId}`);
    ...
    const hintBtn = recreatedPopup.querySelector('.hint-qa-btn');
    ...
    if (hintBtn) {
        hintBtn.addEventListener('click', function() {
            console.log('Q&Aポップアップのヒントボタンがクリックされました');
            const answerElement = recreatedPopup.querySelector('.qa-answer-content');
            toggleHintDisplay(answerElement, this, '💡 ヒント解除', '💡 ヒント！');
        });
    }
    ...
    statusButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (moduleId) {
                qaStatusSystem.setStatus(moduleId, qaId, status);
            } else {
                // フォールバック：2引数形式でsetStatusを呼び出し（内部でmoduleIdを取得）
                console.warn('⚠️ moduleIdが取得できません。フォールバック処理を実行します。');
                qaStatusSystem.setStatus(qaId, status);
            }
            ...
            setTimeout(() => {
                if (window.qaStatusSystem) {
                    console.log(`🎨 Q&Aポップアップ内色更新: Q${qaId} → ${status}`);
...
function toggleAllBlanks(container, reveal) {
    ...
    blanks.forEach(blank => {
        const currentRevealed = blank.dataset.isRevealed === 'true';
        if (reveal && !currentRevealed) {
            window.toggleBlankReveal(blank);
        } else if (!reveal && currentRevealed && blank.dataset.isChecked !== 'true') {
            window.toggleBlankReveal(blank);
        }
    });
}
...
window.createRankBadge = createRankBadge;
