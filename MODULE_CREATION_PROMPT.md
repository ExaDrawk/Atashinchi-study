...
**★AIキャラクター会話でのMermaidグラフ対応（NEW）:**
- AIキャラクター会話内でも```mermaid〜```記法でグラフを描画可能
- リアルタイムでMermaidコードがビジュアル化
- ズーム・パン機能も自動適用
- 学習者の質問に対してキャラクターが図解で回答可能
- 複雑な概念をその場で視覚的に説明
```
...
- **川島**: ユズヒコのクラスメート、14歳。ユズヒコと婚姻する妄想の結果、親族・相続法にドはまり中。プールでの出来事をきっかけにユズヒコに熱烈な片思いをする。親友の山下と「ユズヒコファンクラブ」を結成。恋に一途で情熱的だが、思い込みが激しく行動は空回りしがち。ユズヒコを「タチバナくん」と呼ぶ。片思い中なので丁寧語で話す。主にユズヒコの中学校に登場。
...
このプロンプトに従って、指定された法分野・テーマのモジュールを作成してください。エラーゼロ、教育効果最大を目指して、丁寧に作成をお願いします。

## 🔧 ファイル名競合防止システム

**自動ID生成ルール**：
- 同名ファイルが複数のカテゴリに存在する場合、システムが自動的にカテゴリプレフィックスを付与
- 例：`6.js`が地理学と英語カテゴリに存在 → `地理学-6`、`英語-6`として識別
- モジュール作成時は**元のファイル名のまま**作成してOK（システムが自動処理）

**競合例**：
```
public/cases/地理学/6.js → ID: "地理学-6"
public/cases/英語/6.js   → ID: "英語-6"
```

**重要**：モジュール内の`id`プロパティは元のファイル名ベースで設定し、競合時のプレフィックスはビルドシステムが自動付与します。
